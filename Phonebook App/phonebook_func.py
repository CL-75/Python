...
#  Author: Casey Levy
#
#  File Name:  phonebook_func.py
#
#  Purpose: Demo. Creating the functions for use within a basic phonebook
#           application using Tkinter and practicing Parent/Child class
#           relationships
#
#
#
...
# we're passing in the tkinter frame (master) with 'self' 
def center_window(self, w, h):
    screen_width = self.master.winfo_screenwidth()
    screen_height = self.master.winfo_screenheight()

    # calculating x and y coordinates to paint the app centered on the screen
...
# catching if user clicks "X" in upper right corner on Windows OS
def ask_quit(self):
...
def create_db(self):
    ...
    with conn:
        ...
        # must use commit() to save changes and close DB connection
        conn.commit()
    conn.close()
    first_run(self)
...
def onSelect(self, event):
    ...
    with conn:
        ...
        # returns a tuple and we can slice it into 4 different parts
        for data in varBody:
...
def addToList(self):
    ...
    # normalize data to keep it consistent in the DB
    var_fname = var_fname.strip()    # removes blank spaces before and after user's entry
    var_lname = var_lname.strip()
    var_fname = var_fname.title()
    var_lname = var_lname.title()
    var_fullname = ("{} {}".format(var_fname, var_lname))      # combining our normalized name into a full name
